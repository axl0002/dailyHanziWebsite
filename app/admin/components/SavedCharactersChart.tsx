...
export default function SavedCharactersChart({ filter = 'all', dateRange = 'all' }: { filter?: ProFilter; dateRange?: DateRange }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const { data: raw, error: rpcErr } = await supabase.rpc('saved_characters_histogram', { since_date: since });
            if (rpcErr) {
                console.error('saved_characters_histogram:', rpcErr);
    ...
    if (error) return (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex flex-col items-center justify-center h-[300px]">
            <p className="text-red-600 font-medium">Failed to load Characters Saved</p>
    ...
    if (filter === 'false') return (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 md:col-span-2 lg:col-span-1 flex flex-col items-center justify-center h-[300px]">
            <p className="text-gray-500 font-medium">Characters Saved per User</p>
