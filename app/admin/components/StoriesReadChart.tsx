...
export default function StoriesReadChart({ filter = 'all', dateRange = 'all' }: { filter?: ProFilter; dateRange?: DateRange }) {
    const { data, loading, error, retry } = useRpcData(
        'stories_read_histogram',
        { since_date: sinceFromDateRange(dateRange) },
        parseRow,
    );

    if (loading) return <ChartLoading />;
    if (error) return <ChartError title="Stories Read" error={error} onRetry={retry} />;
    if (filter === 'false') return <ChartProOnlyPlaceholder title="Stories Read per User" />;
