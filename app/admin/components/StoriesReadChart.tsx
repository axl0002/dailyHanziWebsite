...
// Calls the stories_read_by_day RPC — daily paragraph reads from
// user_paragraph_assignments.read_at.
export default function StoriesReadChart({ dateRange = 'all' }: { dateRange?: DateRange }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const { data: raw, error } = await supabase.rpc('stories_read_by_day', { since_date: since });
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 md:col-span-2">
            <div className="flex items-baseline justify-between mb-6">
                <h3 className="text-lg font-bold text-gray-900">Stories Marked as Read</h3>
                                ...
                                if (active && payload && payload.length) {
                                    return (
                                        <div className="bg-white p-3 border border-gray-100 shadow-lg rounded-xl">
                                            ...
                                            <p className="text-sm text-gray-700">{Number(payload[0].value).toLocaleString()} stories read</p>
                                        </div>
                                    );
                                }
                                return null;
