...
// Histogram of users bucketed by how many characters they have "in review" —
// rows in user_seen_characters with strength IN (1, 2). Post-paywall.
export default function InReviewCharactersChart({ filter = 'all', dateRange = 'all' }: { filter?: ProFilter; dateRange?: DateRange }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const { data: raw, error: rpcErr } = await supabase.rpc('in_review_characters_histogram', { since_date: since });
            if (rpcErr) {
                console.error('in_review_characters_histogram:', rpcErr);
    ...
    if (error) return (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex flex-col items-center justify-center h-[300px]">
            <p className="text-red-600 font-medium">Failed to load Characters in Review</p>
    ...
    if (filter === 'false') return (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 md:col-span-2 lg:col-span-1 flex flex-col items-center justify-center h-[300px]">
            <p className="text-gray-500 font-medium">Characters in Review per User</p>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 md:col-span-2 lg:col-span-1">
            <div className="mb-6">
                <div className="flex items-baseline justify-between">
                    <h3 className="text-lg font-bold text-gray-900">Characters in Review per User</h3>
                    <span className="text-xs text-gray-500">{activeUsers.toLocaleString()} of {poolTotal.toLocaleString()} Pro users have ≥1 in review ({rangeLabel})</span>
                </div>
                <p className="text-xs text-gray-500 mt-1">Strength 1–2 in user_seen_characters — still building mastery.</p>
            </div>
                                ...
                                if (active && payload && payload.length) {
                                    ...
                                    return (
                                        <div className="bg-white p-3 border border-gray-100 shadow-lg rounded-xl min-w-[150px]">
                                            <p className="font-semibold text-gray-900 mb-2">{label} characters in review</p>
