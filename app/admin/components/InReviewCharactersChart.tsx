...
// (strength ∈ {1, 2} in user_seen_characters). Backed by
// in_review_characters_histogram RPC. Post-paywall Pro-only display.
export default function InReviewCharactersChart({ filter = 'all', dateRange = 'all' }: { filter?: ProFilter; dateRange?: DateRange }) {
    const { data, loading, error, retry } = useRpcData(
        'in_review_characters_histogram',
        { since_date: sinceFromDateRange(dateRange) },
        parseRow,
    );

    if (loading) return <ChartLoading />;
    if (error) return <ChartError title="Characters in Review" error={error} onRetry={retry} />;
    if (filter === 'false') return <ChartProOnlyPlaceholder title="Characters in Review per User" />;
